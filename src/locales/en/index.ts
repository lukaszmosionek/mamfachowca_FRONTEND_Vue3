import login from './login-en.json'
import register from './register-en.json'
import validation from './validation-en.json'
import footer from './footer-en.json'
import filtering from './filtering-en.json'
import homeTile from './homeTile-en.json'
import contact from './contact-en.json'
import header from './header-en.json'
import other from './other-en.json'
import bookService from './bookService-en.json'
import account from './account-en.json'
import favorites from './favorites-en.json'
import appointment from './appointment-en.json'
import myServices from './myServices-en.json'
import myService from './myService-en.json'
import photoGallery from './photoGallery-en.json'
import pagination from './pagination-en.json'
// import service from './service.json'

export default {
  login,
  register,
  validation,
  filtering,
  homeTile,
  contact,
  footer,
  other,
  bookService,
  account,
  favorites,
  appointment,
  myServices,
  myService,
  photoGallery,
  pagination,
  header
  // service,
}
