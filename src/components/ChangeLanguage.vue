<template>
  <div class="text-center py-4 text-gray-600 flex justify-center items-center">
    <button v-if="locale !== 'en'" @click="changeLanguage('en')" class="flex justify-center items-center gap-2 hover:opacity-50">
      <img alt="Flag EN" src="@/assets/icons/flag-en.svg" width="20" height="20" /> <span class="text-sm">EN</span>
    </button>
    <button v-if="locale !== 'pl'" @click="changeLanguage('pl')" class="flex justify-center items-center gap-2 hover:opacity-50">
