  ...
  <div v-if="services.length">
    <div class="mx-auto bg-white rounded shadow-md overflow-hidden flex mt-5 w-full relative"
      ...
      <div class="flex-1 w-full">
        <div class="md:p-4 p-2 flex flex-col justify-between h-full">
            ...
            <div class="flex flex-col justify-between">
              ...
              <div class="text-gray-600 text-xs md:text-sm" v-if="s?.provider">
                <div class="mt-2 flex"><span class="hidden md:block">Service duration::&nbsp;</span><span>{{ s.duration }} min.</span></div>
                <div class="flex items-center"><span class="hidden md:block">Provider:&nbsp;</span> <RouterLink class="" :to="{ name: 'Profile', params: { userId: s?.provider?.id ?? 0 } }">{{s?.provider?.name }}</RouterLink><RouterLink class="text-lg md:text-2lg" :to="{ name: 'Messages', params: { userId: s?.provider?.id ?? 0 } }">&#9993;</RouterLink></div>
              </div>
            </div>
            <!-- Opis -->
            <!-- Cena i serduszko -->
        ...
        <div @click="toggleFavorite(s.id, $event)" class="absolute top-0 right-2 text-gray-400 hover:text-red-500 cursor-pointer text-4xl">{{ s.is_favorited ? '♥' : '♡' }}</div>
