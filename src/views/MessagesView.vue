          ...
          <div class="mb-4 w-1/4 pr-4">
            ...
            <ul class="space-y-2">
              <li v-for="person in messagedPeople" :key="person.id" class="overflow-hidden text-ellipsis whitespace-nowrap">
                <RouterLink :to="{ name: 'Messages', params: { userId: person.id ?? 0 } }" class="text-blue-500 hover:underline" :class="user.receiverUser.id === person.id ? 'font-bold' : ''" :title="person.email">
                  {{ person.name }}
                </RouterLink>
              </li>
            </ul>
